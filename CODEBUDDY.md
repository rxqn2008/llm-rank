...
此时 NPC 应主动提醒用户（**注意：创建组织 / 创建仓库等操作请在 PC 端（电脑浏览器）完成**）：

1. **创建自己的组织**：在 PC 端登录 <https://cnb.cool>，点击左侧「创建组织」，拥有一个属于自己的组织空间；
2. **创建自己的仓库**：在 PC 端组织下新建仓库（公开或私有均可）；
...
>
> 请按下面步骤操作（请在 **PC 端** 操作）：
> 1. 在 PC 端登录 <https://cnb.cool>，点击左侧「**创建组织**」，创建属于自己的组织；
> 2. 在 PC 端组织下「**创建仓库**」，建议选择**私有**仓库以保证代码安全；
